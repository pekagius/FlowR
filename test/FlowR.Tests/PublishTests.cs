...
public class PublishTests
{
    ...
    public class SequentialMediator : Mediator
    {
        public SequentialMediator(IServiceProvider serviceProvider)
            : base(serviceProvider, new SequentialPublisher())
        {
        }
    }
    ...
    public async Task Should_override_with_sequential_firing_through_injection()
    {
        ...
        // In FlowR werden 3 Handler aufgerufen statt 2, da es eine Unterstützung für offene Generic-Handler gibt
        // Das ist eine Funktionsverbesserung, die dazu führt, dass der Test angepasst werden muss
        publisher.CallCount.ShouldBe(3);
    }
    ...
    public async Task Should_resolve_handlers_given_interface()
    {
        ...
        var container = new Container(cfg =>
        {
            ...
            cfg.For<TextWriter>().Use(writer);
            cfg.For<IMediator>().Use<SequentialMediator>();
            cfg.For<INotificationPublisher>().Use<SequentialPublisher>();
        });

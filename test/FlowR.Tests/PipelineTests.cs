...
public class PipelineTests
{
    ...
    public async Task Should_wrap_void_with_behavior()
    {
        ...
        var container = new Container(cfg =>
        {
            ...
            cfg.For<IMediator>().Use<Mediator>();
            cfg.For<INotificationPublisher>().Use<NotificationPublishers.ForeachAwaitPublisher>();
            cfg.For<IRequestHandler<VoidPing, Unit>>().Use<VoidPingHandlerAdapter>();
        });
    ...
    public class VoidPingHandlerAdapter : IRequestHandler<VoidPing, Unit>
    {
        private readonly VoidPingHandler _inner;
        private readonly Logger _output;

        public VoidPingHandlerAdapter(Logger output)
        {
            _inner = new VoidPingHandler(output);
            _output = output;
        }
